...
/*这是一个wifi工具类，可以用于自动连接wifi，断开wifi，获取wifi的各种信息*/
...
//获取目前连接的wifi 名称（SSID）
    ...
    public boolean openWifi() {//先检测wifi是否启用，没有启用就打开wifi
    ...
    public boolean startScan() {//wifi扫描,判断wifi有没有发射器的热点，有返回真没有返回假
 ...
 boolean tmp=false;
        Log.d("oncreate", "wifi扫描结果:" + mWifiList.size());
        Log.d("oncreate", "wifi扫描结果是否为空:" + mWifiList.isEmpty());
        if (mWifiList != null) {
            ...
            for (int i = 0; i < mWifiList.size(); i++) {
                ...
                if (result.SSID.contains("IYK")) {
                    IYKSSID=result.SSID;
                    Log.d("oncreate", "发射器的SSID:" + IYKSSID);
                    tmp=true;
